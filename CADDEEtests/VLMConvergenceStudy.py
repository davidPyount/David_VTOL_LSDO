...
def define_vlm_analysis(caddee: cd.CADDEE):
    ...
    #if we re-assign the meshes to the same dictionary space in each iteration it shouldn't cause issues.
...
if __name__ == "__main__": #I like doing this because it makes it clear where the main executiom begins and also I can collapse it : )
    ...
    for i in range(starti,starti+iterationi-1):
        for j in range(startj,startj+iterationj-1):
            num_chordwise = i
            num_spanwise = j
            if num_spanwise % 2 == 0:
                 #Location of VLM mesh generation:
                # tail_chord_surface = cd.mesh.make_vlm_surface(
                #     wing_comp=h_tail,
                #     num_chordwise=tail_chordwise_panels, 
                #     num_spanwise=tail_spanwise_nodes,
                #     plot=False 
                ...
                total_force[i,j] = np.linalg.norm(vlm_output.total_force[0].value)
        #plt.plot(np.linspace(startj,startj+iterationj,np.size(total_force[i,:],0)),total_force[i,:])
        #plt.show()
