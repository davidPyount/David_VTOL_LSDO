...
filename = os.path.join(dirname, 'shark2.stp')
mark2_geom = cd.import_geometry(filename)
plotting_elements = mark2_geom.plot(show=plotting, opacity=0.5, color='#FFCD00')

# define INITIAL values for design parameters

# fuselage
fuselage_len = 1.25 * units.length.foot_to_m
...
# span = 6 * units.length.foot_to_m
# wingchord = 12/12 * units.length.foot_to_m
...
# VLM nodes
# 12ish chordwise should be fine 
wing_chordwise_panels = 12
# 20 to 30 spanwise is best
wing_spanwise_panels = 30
h_tail_chordwise_panels = 8
# 20 to 30 spanwise is best 
h_tail_spanwise_panels = 30
...
def define_base_config(caddee : cd.CADDEE):
    ...
    fuselage_length = csdl.Variable(name="fuselage_length", value=fuselage_len)
    ...
    main_spar_length.set_as_design_variable(lower=0.875*main_spar_len, upper=4*units.length.foot_to_m, scaler=1)
    ...
    wing_geometry = aircraft.create_subgeometry(search_names=["Wing"], 
                                                ignore_names=["34","35","24","25","28","29","30","31"])
    # wing_geometry.plot()
    # wing_geometry = aircraft.create_subgeometry(search_names=["Wing"])
    aspect_ratio = csdl.Variable(name="wing_aspect_ratio", value = wing_AR)
    wing_area = csdl.Variable(name="wing_area", value = wing_S)
    wing_root_twist_delta = csdl.Variable(name="wing_root_twist_delta", value=np.deg2rad(0))
    wing_tip_twist_delta = csdl.Variable(name="wing_tip_twist_delta", value=np.deg2rad(0))
    
    # Set design variables for wing
    aspect_ratio.set_as_design_variable(lower=0.5 * wing_AR, upper=8, scaler=1/5) # initial value is 5.33
    wing_area.set_as_design_variable(lower=0.5 * wing_S, upper=2 * wing_S, scaler=4) # initial value is 0.278
    wing_root_twist_delta.set_as_design_variable(lower=np.deg2rad(-1.5), upper=np.deg2rad(1.5), scaler=40) # 1 deg = 0.017 rad
    wing_tip_twist_delta.set_as_design_variable(lower=np.deg2rad(-3), upper=np.deg2rad(1.5), scaler=25)
    thickness_to_chord_wing = csdl.Variable(name="toc", value=wing_tc)
    
    wing = cd.aircraft.components.Wing(
        AR=aspect_ratio, S_ref=wing_area,
        root_twist_delta=wing_root_twist_delta,
        tip_twist_delta=wing_tip_twist_delta, geometry=wing_geometry,
        thickness_to_chord=thickness_to_chord_wing, thickness_to_chord_loc=0.3
    )
    ...
    h_tail_root_twist_delta = csdl.Variable(name="h_stab_root_twist_delta", value=np.deg2rad(0))

    # Set design variables for wing
    h_tail_AR.set_as_design_variable(lower=2, upper=2 * h_stab_AR, scaler=1/3) # inital value is 3
    h_tail_area.set_as_design_variable(lower=0.5 * h_stab_S, upper=2 * h_stab_S, scaler=20) # initial value is 0.048
    h_tail_root_twist_delta.set_as_design_variable( lower=np.deg2rad(-2.5), upper=np.deg2rad(1.5), scaler=25) # 1 deg = 0.017 rad  
    thickness_to_chord_h_tail=csdl.Variable(name="h_stab_tc", value=h_stab_tc)
    ...
    h_tail = cd.aircraft.components.Wing(
        AR=h_tail_AR, S_ref=h_tail_area, 
        geometry=h_tail_geometry, root_twist_delta=h_tail_root_twist_delta, tip_twist_delta=h_tail_root_twist_delta,
        thickness_to_chord=thickness_to_chord_h_tail, thickness_to_chord_loc=0.3
    )
    ...
    # nosecone is used later to connect component geometries
    ...
    cruise_prop = cd.aircraft.components.Rotor(radius=4*units.length.inch_to_m, geometry=cruise_prop_geom, 
                                               compute_surface_area=False, skip_ffd=True) #make CADDEE component
    ...
    #These are taken from other code, make them accurate for us.
    cruise_prop_mesh.chord_profile = csdl.Variable(shape=(num_radial, ), 
                                    value=np.linspace(1*units.length.inch_to_m, 0.2*units.length.inch_to_m, num_radial))
    #Assign bladeAD rotor mesh to caddee rotor mesh container
    ...
    base_config.setup_geometry(plot=plotting)
...
def define_conditions(caddee: cd.CADDEE):
    ...
    pitch_angle = csdl.Variable(name="pitch_angle", value=np.deg2rad(0))
    # pitch_angle.set_as_design_variable(lower=np.deg2rad(-5), upper=np.deg2rad(5), scaler=15)
...
def define_mass_properties(caddee : cd.CADDEE,vlm_output):
    ...
    battery_x.set_as_design_variable(lower=-15*cd.Units.length.inch_to_m, upper=-6*cd.Units.length.inch_to_m, scaler=5) # scaler of 5 is good for 6-15 range

    # ballast = aircraft.comps["ballast"]
    # ballast_mass = csdl.Variable(name="ballast_mass", value=0.1*units.mass.pound_to_kg)
    # # ballast_mass.set_as_design_variable(lower=0, upper=0.5*units.mass.pound_to_kg, scaler=5)
    # ballast_mass.set_as_design_variable(lower=0, scaler = 2)
    # ballast.quantities.mass_properties.mass = ballast_mass


    # # ballast_x = csdl.Variable(name="ballast_x", value=0)
    # # ballast_position = csdl.Variable(name="battery_position", value=np.zeros(3))
    # ballast_position = csdl.Variable(name="battery_position", value=np.array([7*units.length.inch_to_m, 0, 0]))
    ...
    # # ballast_x.set_as_design_variable(lower=0, upper=7*cd.Units.length.inch_to_m, scaler=8) 

    # ballast_position = csdl.Variable(name="ballast_position", value=np.array([0, 0, 0]))
    # ballast.quantities.mass_properties.cg_vector = ballast_position

    wing : cd.aircraft.components.Wing = aircraft.comps["wing"]

    wing_TE_center = wing.TE_center
    wing_LE_center = wing.LE_center

    wing_qc = 0.75 * wing_LE_center + 0.25 * wing_TE_center
    wing_center = 0.5 * wing_LE_center + 0.5 * wing_TE_center
    ...
    wing_span.name = "wing_span"
    wing_span.set_as_constraint(upper = 6 * cd.Units.length.foot_to_m, scaler=0.5) # 6 ft = 1.8 m
    ...
    if seth_wing_volume:
        ...
        wing.quantities.mass_properties.cg_vector = 0.56 * wing_LE_center + 0.44 * wing_TE_center # CG is around 44.4% of chord for 4412
    else:
    ...
    # fuselage_length = csdl.norm(fuselage.nose_point - fuselage.tail_point)
    fuse_volume = fuselage_len * ((fuse_h*fuse_w) - (fuse_h-2*fuse_t)*(fuse_w-2*fuse_t)) + fuselage_len*(fuse_w/2)*fuse_t # include extra bit on bottom
    ...
    # h_tail_LE_center = h_tail.geometry.evaluate(h_tail._LE_center)
    # h_tail_TE_center = h_tail.geometry.evaluate(h_tail._TE_center)
    h_tail_LE_center = h_tail.LE_center
    h_tail_TE_center = h_tail.TE_center
    ...
    h_tail.quantities.mass_properties.cg_vector = 0.575 * h_tail_LE_center + 0.425 * h_tail_TE_center
    ...
    v_tail.quantities.mass_properties.cg_vector = 0.575 * h_tail_LE_center + 0.425 * h_tail_TE_center
    ...
    half_boom_length = (wing_chord + lift_rotor_d*1.1)/2
    ...
    # Volume = pi/4 * (0.75^2 - 0.5^2) * 0.0254^2 * 0.9144 = 1.014e-4 m^3
    # Density = 0.0864 kg / 1.014e-4 m^3 = 852.1 kg/m^3
    # note: a lot less than the other tube used in the wings
    half_boom_density = 852.1 # kg/m^3
    half_boom_mass = half_boom_volume * half_boom_density

    left_boom_assembly = aircraft.comps["left_boom_assembly"]
    left_boom_assembly_mass = csdl.Variable(name="boom_assembly_mass", value = 
                    wing_boom_mount_mass_val + (half_boom_mass.value + lift_motor_mass_val + lift_motor_mount_mass_val + lift_rotor_mass_val)*2)
    ...
    left_boom_assembly_CG = left_boom_assembly_CG.set(csdl.slice[0], wing_center[0])
    ...
    right_boom_assembly_CG = right_boom_assembly_CG.set(csdl.slice[0], wing_center[0])
    ...
    main_spar_nose_point = main_spar.geometry.evaluate(main_spar._nose_point)
    main_spar_tail_point = main_spar.geometry.evaluate(main_spar._tail_point)
    
    main_spar_length = csdl.norm(main_spar_nose_point - main_spar_tail_point)
    ...
    main_spar.quantities.mass_properties.cg_vector = (main_spar_nose_point + main_spar_tail_point)/2
    ...
    tail_mount_CG = main_spar_tail_point + tail_mount_half_length
    ...
    # total_aircraft_mass.set_as_constraint(upper=7.5*cd.Units.mass.pound_to_kg, scaler=1/7) 

    print(f"The total aircraft mass is {base_config.system.quantities.mass_properties.mass.value}")
    print(f"The wing quarter chord coordinates are {wing_qc.value}")
    # print(base_config.system.quantities.mass_properties.inertia_tensor.value)
    print(f"The CG location is {base_config.system.quantities.mass_properties.cg_vector.value}")
...
def run_beam(caddee: cd.CADDEE, vlm_output):
    ...
    beam_thickness = csdl.Variable(value=0.0015875)
    ...
    # r.set_as_constraint(lower=0, scaler = 50, adder=1)
    r.set_as_constraint(lower=0, scaler = 1, adder = 1)

    beam_1_def_mesh = wing_spar_nodes + beam_displacement

    # plt.figure()
    # plt.grid()
    # plt.plot(wing_spar_nodes.value[:, 1], beam_displacement.value[:, 2], color='black', linewidth=2)
    # plt.scatter(wing_spar_nodes.value[:, 1], beam_displacement.value[:, 2], zorder=10, edgecolor='black', s=50, color='green')
    # plt.xlabel('y (m)')
    # plt.ylabel('z (m)')
    # plt.show()
...
def define_analysis(caddee: cd.CADDEE, vlm_output):
    ...
    rpm.set_as_design_variable(lower=100,upper=14328,scaler=1/7000)
    # rpm.set_as_design_variable(lower=999,upper=14328,scaler=1/10000)
    ...
    total_forces_cruise.set_as_constraint(upper=0.01, lower=-0.01, scaler=100)
    total_forces_cruise.name = "total_forces_cruise"
    total_moments_cruise.set_as_constraint(upper=0.01, lower=-0.01, scaler=100)
    total_moments_cruise.name = "total_moments_cruise"

    #This is how the trim residual is set.
    # accel_norm_cruise.set_as_constraint(upper=0.01, lower=-0.01, scaler=100)
        # Setting force equilibrium constraints
    # force_norm = csdl.norm(total_forces_cruise)
    # moment_norm = csdl.norm(total_moments_cruise)

    # Hello world, this is a test of the force and moment residuals
    print("why is injured knee warmer?")
    print("because it is inflamed")
    print("Im sorry - that was a bad joke")
    print("I will see myself out")
    print("goodbye (#sethrules)")
    ...
    # drph.set_as_constraint(lower=0,scaler=50)
    # drph.set_as_constraint(lower=0.02,upper=0.045,scaler=1/0.04)
    drph.name = "damping_ratio_phugoid"
    drsp = long_stability_results.damping_ratio_short_period
    # drsp.set_as_constraint(lower=0,scaler=10)
    drsp.set_as_constraint(lower=0.1,upper=2,scaler=5)
    drsp.name = "damping_ratio_short_period"
    print(f"initial damping ratio phugoid {drph.value}")
    print(f"initial damping ratio short period {drsp.value}")
    ...
    ref_point_x = csdl.Variable(value=0)

    vlm_NP_output = vlm_solver(
    lattice_coordinates, 
    lattice_nodal_velocities, 
    atmos_states=cruise.quantities.atmos_states,
    airfoil_Cd_models=[None, None],
    ...
    airfoil_Cp_models=[None, None],
    airfoil_alpha_stall_models=[None, None],
    ref_point=ref_point_x)

    initial_M_pitch = vlm_NP_output.total_moment[0][1]
    pitch = cruise.parameters.pitch_angle

    dM_dalpha = csdl.derivative(initial_M_pitch, pitch)
 
    # find x that gives zero delta_M_pitch
    stability_solver = csdl.nonlinear_solvers.BracketedSearch(name="stability_solver", max_iter=200)
    stability_solver.add_state(state=ref_point_x, residual=dM_dalpha.flatten(), bracket=(-10., 0.))
    stability_solver.run()

    neutral_point = ref_point_x
    print(f"neutral point {neutral_point.value}")

    # compute static margin (note that x-axis is positive forward)
    aircraft_cg = base_config.system.quantities.mass_properties.cg_vector
    static_margin = (aircraft_cg[0] - neutral_point) / mac

    static_margin.name = "static_margin"
    static_margin.set_as_constraint(lower=0.1, upper=0.2, scaler=5)
...
#I like doing this because it makes it clear where the main executiom begins and also I can collapse it : )
if __name__ == "__main__": 
    ...
    long_stability_results = define_analysis(caddee=caddee,vlm_output=vlm_output)

    csdl.save_optimization_variables()
    csdl.inline_export("design_vars.hdf5")
    ...
    # optimizer = IPOPT(problem, solver_options={'max_iter': 1000, 'tol': 1E-6}, turn_off_outputs=True)
    ...
    print(f"The aircraft mass is {caddee.base_configuration.system.quantities.mass_properties.mass.value}")
    # print(f"The wing's quarter chord coordinates are {wing_qc.value}")
    print(f"The final phugoid mode is {long_stability_results.damping_ratio_phugoid.value}")
    print(f"The final short period mode is {long_stability_results.damping_ratio_short_period.value}")
